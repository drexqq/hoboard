...
public class Ask_Dao {
	...
	public List<Ask_Dto> getAskList(String id) {
		String query = " SELECT SEQ, AUTH, ID, TITLE, CONTENT, COMM, WDATE FROM ASK_TABLE "
					+ " WHERE ID = ?";
	...
	public int getAllAsk(String choice, String searchWord) {
	...
	public List<Ask_Dto> getAskPagingList(String choice, String searchWord, int page) {
		...
		/*
		 	...
		 	4. 범위설정 1~10까지
		 */
		String sql = " SELECT SEQ, AUTH, ID, TITLE, CONTENT, COMM, WDATE "
				+ " FROM ";
		
		sql += "(SELECT ROW_NUMBER()OVER(ORDER BY SEQ DESC) AS RNUM, " + 
					" SEQ, AUTH, ID, TITLE, CONTENT, COMM, WDATE " + 
					" FROM ASK_TABLE ";
		...
		try {
			...
			System.out.println("3/6 getAskPagingList success");
				...
				while(rs.next()) {
					int j = 1;
					Ask_Dto dto = new Ask_Dto(rs.getInt(j++), 
												rs.getInt(j++), 
												rs.getString(j++), 
												rs.getString(j++), 
												rs.getString(j++), 
...
//	}
	...
	public boolean ask_write(Ask_Dto dto) {
		...
		String sql = " INSERT INTO ASK_TABLE "
					+ " ( SEQ, AUTH, ID, TITLE, CONTENT, COMM, WDATE )  "
	...
	//글 삭제
		public boolean ask_delete(int seq) {
			...
			String sql = " DELETE FROM ASK_TABLE "
						+ " WHERE SEQ=? ";
	...
	// 업데이트
		public boolean ask_update(int seq, String title, String content) {
			String sql = " UPDATE ASK_TABLE SET "
					+ " TITLE=?, CONTENT=? "
					+ " WHERE SEQ=? ";
