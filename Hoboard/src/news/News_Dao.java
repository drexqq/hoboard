...
private static News_Dao n_dao = new News_Dao();
	...
	public boolean news_write(News_Dto dto) {
		...
		String sql = " INSERT INTO NEWS "
					+ " ( NEWS_SEQ, ID, TITLE, CONTENT,VIEWCOUNT, WDATE, REF,STEP,DEPTH)  "
	...
	public List<News_Dto> getnews_list(String choice, String searchWord) {
		...
		try {
			...
			while(rs.next()) {
				int i = 1;
				News_Dto dto = new News_Dto(rs.getInt(i++), 
											rs.getString(i++), 
											rs.getString(i++), 
											rs.getString(i++), 
											rs.getInt(i++), 
...
=======
			  ...
			  sql += " ORDER BY NEWS_SEQ DESC) "; // 최신 날짜 순으로 정렬
>>>>>>> 7d70662a62da5d50c8e89ec30ab1f4ff0b18fa3b
			...
			try {
				...
				System.out.println("3/6 getNewsPagingList success");
					...
					while(rs.next()) {
						int j = 1;
						News_Dto dto = new News_Dto(rs.getInt(j++), 
													rs.getString(j++), 
													rs.getString(j++), 
													rs.getString(j++), 
													rs.getInt(j++), 
